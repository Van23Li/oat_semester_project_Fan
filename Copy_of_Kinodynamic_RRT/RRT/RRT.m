...
while failedAttempts <= cfg.maxFailedAttempts && counter_rand <= cfg.maxSample
    ...
    if cfg.origin 
        ...
        [~, ~, M, ~] = nearest(@Dist,sample, RRTree, cfg);
        if M > cfg.stepsize
            sample(1:cfg.dim) = closestNode(1:cfg.dim) + (sample(1:cfg.dim)-closestNode(1:cfg.dim))/norm(sample(1:cfg.dim)-closestNode(1:cfg.dim), 2) * cfg.stepsize;
        else
            sample = sample;  % [q1; q2]
        end
        newPoint = Steer(closestNode, sample, T, cfg);
