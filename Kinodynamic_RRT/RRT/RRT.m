...
%                 line(handle.ax_h2,path(2,1:2),path(1,1:2),'color',[1 0 0],'linewidth',2);
            ...
            if cfg.display3 == 1
                ...
                tmp_close = calc_fk(path(1:cfg.dim,2),cfg.r,cfg.d,cfg.alpha,cfg.base);
    ...
    if cfg.kinodynamic
        ...
        for i = 1 : size(path,2)-1
            ...
            q1_list = [q1_list, y];
            plot(ax_h5,x,y,'color',[0 0 1]);
            q_end = y(end);
            v_end = v_end + a_2(1).* (x(end) - t_end);
            t_end = t_end + t_2(1);
        end
        ...
        for i = 1 : size(path,2)-1
            ...
            q2_list = [q2_list, y];
            plot(ax_h6,x,y,'color',[0 0 1]);
            q_end = y(end);
            v_end = v_end + a_2(2).* (x(end) - t_end);
            t_end = t_end + t_2(2);
        end
        
        % plot interpolated path in C-space
        a=2
        ...
        if cfg.display3 == 1
            for i = 1:size(path,2)
                ...
                hold on
                handle.anime = create_r(handle.ax_h3,path(1:cfg.dim,i), cfg.r, cfg.d, cfg.alpha, cfg.base);
                pause(0.2);
                hold off
            end
        end
