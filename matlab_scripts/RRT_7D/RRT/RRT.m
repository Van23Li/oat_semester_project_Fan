function path = RRT (RandomSample, Dist, obs, cfg, Name_result)
...
if pathFound
    ...
    result(end).Counter_rand = counter_rand;
    result(end).NumOfNodes = size(RRTree,2);
    result(end).PathLength = pathLength;
...
%                 pause(0.2);
    ...
    load(Name_result);
    result(end).Time = Time;
    result(end).Counter_rand = 10000;
    result(end).NumOfNodes = size(RRTree,2);
    result(end).PathLength = 0;
