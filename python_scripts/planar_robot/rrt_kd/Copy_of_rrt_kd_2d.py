...
dim = 2
X_dimensions = np.array([(q_min[0], q_max[1]), (q_min[0], q_max[1]), (v_min[0], v_max[1]), (v_min[0], v_max[1])])
                            # dimensions of Search Space
x_init = (-2.5, -1, -1, -2)  # starting location
x_goal = (2.7, 0, 1, 2)  # goal location
V_limits = np.array([(v_min[0], v_max[1]), (v_min[0], v_max[1])])  # dimensions of Velocity
A_limits = np.array([(a_min[0], a_max[1]), (a_min[0], a_max[1])])  # dimensions of Acceleration
...
Obstacles = generate_random_obstacles(X, x_init, x_goal, n, kd = True)
# create rrt_search
rrt = RRTKd(X, V_limits, A_limits, Q, x_init, x_goal, max_samples, r, prc, kd = True)
# path = rrt.rrt_search()
path = rrt.rrt_kd()

# plot
plot = Plot_kd("rrt_2d_with_random_obstacles")
